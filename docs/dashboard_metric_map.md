...
- `totals.loopLines` lists the circular-loop layer: `electricityValue`, `heatValue`, `carbonCredits`, `compostRevenue`, `loopFeedSaving`, and the negative `waterCost`, `coolingCost`, `diseaseCost`. `profit = herdProfit + loopNet`.
- `processorRevenueNotInProfit` and `processorEnergyCostNotInProfit` are the processor's product sales and processing energy. The processor is treated as a separate business, so both are reported (also in `modelDetails` and the daily export) and neither is in profit; profit keeps the workbook milk component sales. The processor runs with L4 so whey and waste milk exist for the by-product loop.
...
| Processor energy cost (not in profit) | `ctx.daily_records` | `processor_energy_cost_not_in_profit` | daily/run | currency | sum | `EXISTING` | Reported beside processor revenue; excluded from farm profit (separate business). |
