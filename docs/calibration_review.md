...
| Farm profit | Workbook herd economics (every Table 4 line) plus an itemized loop layer: electricity value, heat value, carbon credits, compost revenue, by-product revenue, loop feed saving, minus water, cooling, processing energy and disease cost. | Workbook Tables Table 4; Blueprint for loop items |
| Processor revenue | Kept out of profit (`processor_revenue_not_in_profit`); farm revenue keeps the workbook milk component sales because the model has no product-side costs. The processor still runs with L4 so whey and waste milk exist. | Workbook precedence |
...
- With product sales outside profit, L4 now nets about -$64 per cow per year for the default farm: a $55 feed saving against $118 of processing energy (`dairy_processor.processing_energy_kwh_per_l_milk`, 0.08 kWh/L, implementation). Charging the processing energy of a plant whose revenue is excluded may need a policy decision.
