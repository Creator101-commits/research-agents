...
Forty target controls are mapped to Python inputs, including herd size, duration, seed, feed cost, lactation and body-weight parameters, manure separator split, biogas and compost yield, solar, methane factor, grid emission factor, cull rate and milk-processing shares; each one changes the run. Target controls without an equivalent model input stay visible and disabled with the tooltip "Not used by the Python model" (the full lists are in `docs/dashboard_metric_map.md`). The Comparison page runs the 16 loop combinations on the server and displays the saved eight-seed Excel parity results. The ROI page keeps the target's equipment cards but takes every price, factor and flow the model has from the run; the remaining catalogue constants are listed as scenario estimates, not farm quotes. Cow Ranking uses retained model cow histories. Parameters includes read-only model details.

Net profit is the workbook herd economics plus an itemized circular-loop layer. Feed is charged on all intake at the workbook prices (Stats_MAST rows 28-29), including home-grown feed, and the L4 by-product feed credit (dry matter of whey and waste milk) lowers that charge; L1 returns nutrients to cropland rather than feed. The processor is treated as a separate business: its product sales and processing energy are shown separately and are not part of profit (milk cooling stays a farm cost). Electricity is valued only up to the farm demand, with surplus kWh reported; heat is reported but not valued because no farm heat demand is configured; no solar capacity is configured by default. These limits are stated on the dashboard cards. Water, nutrient, and byproduct labels retain their source units; unavailable values are shown as unavailable rather than converted without a coefficient.
...
The L3 energy balance reports both net renewable electricity and recoverable CHP heat. The default heat coefficient is a source-linked screening ratio derived from EPA AgSTAR efficiency guidance. A project-specific CHP coefficient and usable daily heat demand should replace the default before assigning heat savings. Until a scenario sets `farm_heat_demand_mj_per_day`, heat value is 0 (Blueprint Energy 8.5), and only electricity up to `energy.farm_energy_demand_kwh_per_day` is valued; the surplus is reported as `surplus_energy_kwh`.
...
| `l1_nutrient_loop_enabled` | bool | calibration default (`true`) | Compost the separator solids and return compost N/P/K to cropland |
...
- **Per-cow lifecycle** tracks age, parity, days in milk, body condition, body weight (parity baseline 540 to 680 kg plus the daily intake residual within a lactation), rumen pH, SARA, pregnancy, calving, mortality, and per-cow trait vectors inherited from genetics. Heat stress (THI bands at 68 and 72) and SARA (sustained low rumen pH) reduce DMI and milk yield.
...
- **Manure management** includes collection efficiency, a separator split (35% solids to compost while L1 is on, 65% liquid to the digester while L3 is on, otherwise to storage), storage inventory with overflow, NPK accounting per route (digester/compost/storage), digester co-feed assembly (manure + grass + food waste), thermochemical valorisation, and processor residue integration.
